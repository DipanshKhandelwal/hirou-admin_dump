...
import { addTaskReport, editTaskReport } from "../../../services/apiRequests/taskReports";
    ...
    >
      ...
      <ModalContent>
        ...
        <Formik
          enableReinitialize
          initialValues={{
            task_collection_point: selectedTaskReport?.task_collection_point ?? '',
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              formData.append('task_collection_point', String(values.task_collection_point))
        ...
        >
          {({
            ...
            <form onSubmit={handleSubmit}>
              <ModalBody pb={6}>
                <FormControl id="task_collection_point" isInvalid={!!(errors.task_collection_point && touched.task_collection_point)} >
                  <FormLabel htmlFor="task_collection_point" >Task collection point</FormLabel>
                  <InputGroup marginY={2} >
                    <Field name="task_collection_point" >
                      {({ field, form }: { field: any, form: any }) => (
                        <Select {...field} placeholder="Select task collection point" id="task_collection_point" name="task_collection_point" >
                          {taskRoute.task_collection_point.map((tcp: ITaskCollectionPoint) => (
                            <option key={tcp.id} value={tcp.id} >{tcp.name}</option>
                          ))}
                        </Select>
                      )}
                    </Field>
                  </InputGroup>
                  <FormErrorMessage>{errors.task_collection_point && touched.task_collection_point}</FormErrorMessage>
                </FormControl>
                <FormControl id="report_type" isInvalid={!!(errors.report_type && touched.report_type)} >
                  <FormLabel htmlFor="report_type" >Report Type</FormLabel>
                  <InputGroup marginY={2} >
                    <Field name="report_type" >
                      {({ field, form }: { field: any, form: any }) => (
                        <Select {...field} placeholder="Select report type" id="report_type" name="report_type" >
                          {reportTypes.map((reporType: ITaskReportType) => (
                            <option key={reporType.id} value={reporType.id} >{reporType.name}</option>
                          ))}
                        </Select>
                      )}
                    </Field>
                  </InputGroup>
                  <FormErrorMessage>{errors.report_type && touched.report_type}</FormErrorMessage>
                </FormControl>
                  ...
                  <InputGroup marginY={2} >
                    {(values.image || selectedTaskReport?.image) && (
                      <Image
                        ...
                        boxSize="150px"
                        objectFit="cover"
                        src={values.image ? URL.createObjectURL(values.image) : selectedTaskReport?.image}
                        alt="image"
                      />
                    )}
