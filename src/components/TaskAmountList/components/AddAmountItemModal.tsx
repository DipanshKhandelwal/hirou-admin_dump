...
import { addTaskAmountItem, editTaskAmountItem } from "../../../services/apiRequests/taskAmountItems";
...
export const AddAmountItemModal = (props: AddAmountItemModalProps) => {
  ...
  const { isOpen, onClose, taskAmount, taskRoute, selectedTaskAmountItem } = props
      ...
      <ModalContent>
        ...
        <Formik
          ...
          validate={(values) => {
            ...
            if (values.vehicle_weight === 0 || !isNumeric(values.vehicle_weight)) {
        ...
        >
          {({
            ...
            <form onSubmit={handleSubmit}>
              <ModalBody pb={6}>
                <FormControl id="garbage" isInvalid={!!(errors.garbage && touched.garbage)} >
                  <FormLabel htmlFor="garbage" >Garbage</FormLabel>
                  <InputGroup marginY={2} >
                    <Field name="garbage" >
                      {({ field, form }: { field: any, form: any }) => (
                        <Select {...field} placeholder="Select garbage" id="garbage" name="garbage" >
                          {grabagesList.map((grabageItem: IGarbage) => (
                            <option key={grabageItem.id} value={grabageItem.id} >{grabageItem.name}</option>
                          ))}
                        </Select>
                      )}
                    </Field>
                  </InputGroup>
                  <FormErrorMessage>{errors.garbage && touched.garbage}</FormErrorMessage>
                </FormControl>

                <FormControl isInvalid={!!(errors.gross_weight && touched.gross_weight)}>
                ...
                <FormControl isInvalid={!!(errors.vehicle_weight && touched.vehicle_weight)}>
